...
String mainPage() {
  ...
  <head>
    ...
    <style>
      ...
      .nav-item:hover { background: #2196f3; }
      .active-tab { background: #0d47a1 !important; }
  ...
  <body>
    ...
    <!-- NAVBAR -->
    <div class="navbar">
      <div class="nav-item active-tab" onclick="openTab('home')">🏠 Trang chủ</div>
      <div class="nav-item" onclick="openTab('devices')">🔌 Thiết bị</div>
      <div class="nav-item" onclick="openTab('settings')">⚙️ Cài đặt</div>
    </div>
    ...
    <div id="home" class="page show">
      ...
      <div class="card">
        <h2>💡 Theo dõi ánh sáng (Lux)</h2>
        <canvas id="chartL" height="150"></canvas>
      </div>

      <div class="card">
        <h3>📌 Trạng thái hệ thống:</h3>
        <div id="sysStatus">Đang tải...</div>
      </div>

      <div class="card">
        <h3>🤖 AI dự đoán:</h3>
        <div id="aiStatus">Đang tải...</div>
      </div>
    </div>
    ...
    <div id="devices" class="page">
      <div class="card">
        ...
        <button id="led1Btn" class="device-btn off" onclick="toggleLED(1)">LED 1</button>
    ...
    <div id="settings" class="page">
      <div class="card">
        ...
        <button onclick="window.location='/settings'" style="padding:12px; border-radius:8px;">Mở trang cài đặt WiFi</button>
      </div>
    </div>
    ...
    <script>
      ...
      const chartTH = new Chart(ctxTH, {
        ...
        data: {
          labels: [],
          datasets: [
            { label: "Nhiệt độ (°C)", data: [], borderColor: "red", tension: 0.3 },
            { label: "Độ ẩm (%)", data: [], borderColor: "blue", tension: 0.3 }
          ]
        },
      ...
      const chartL = new Chart(ctxL, {
        ...
        data: {
          labels: [],
          datasets: [
            { label: "Ánh sáng (Lux)", data: [], borderColor: "orange", tension: 0.3 }
          ]
        },
        options: { responsive: true }
      });
      ...
      function toggleLED(id){
        ...
        .then(r=>r.json())
        .then(j=>{
          document.getElementById('led1Btn').className = "device-btn " + (j.led1 === "ON" ? "on" : "off");
      ...
      function updateSensors(){
        ...
        .then(d=>{
          ...
          chartTH.data.labels.push(t);
          chartTH.data.datasets[0].data.push(d.temp);
          chartTH.data.datasets[1].data.push(d.hum);
          ...
          chartL.data.labels.push(t);
          chartL.data.datasets[0].data.push(d.light);
          if(chartL.data.labels.length > 15){
            chartL.data.labels.shift();
            chartL.data.datasets[0].data.shift();
          }
          chartL.update();
...
void handleSensors()
{
  ...
  String json = "{";
  json += "\"temp\":" + String(glob_temperature, 2) + ",";
  json += "\"hum\":" + String(glob_humidity, 2) + ",";
  json += "\"light\":" + String(glob_light, 2) + ",";
  json += "\"ai_result\":" + String(last_inference, 3);
  json += "}";
  server.send(200, "application/json", json);
}
